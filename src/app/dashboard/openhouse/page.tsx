...
export default function OpenHousePage() {
  ...
  return (
    <div className="animate-fade-in pb-8">
      ...
      {/* Add Visitor Button */}
      <button onClick={() => setShowForm(true)} className="w-full py-4 mb-6 text-lg font-semibold bg-primary-500 text-dark-bg rounded-xl hover:bg-primary-400 transition-colors">
      ...
      <div className="space-y-3">
        {visitors.length === 0 ? (
          ...
          <>
            {visitors.map(v => (
              <div key={v.id} className="bg-gradient-to-br from-dark-card to-[#1F1F1F] rounded-xl p-4 border border-dark-border">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4 backdrop-blur-sm overflow-y-auto">
          <div className="bg-gradient-to-br from-dark-card to-[#1F1F1F] rounded-2xl p-6 max-w-lg w-full border border-dark-border my-8">
            <h2 className="font-playfair text-xl text-primary-400 mb-6">✍️ Visitor Sign-In</h2>
            ...
            <div className="flex gap-4 justify-end">
              ...
              <button onClick={addVisitor} className="px-6 py-3 text-sm font-semibold bg-primary-500 text-dark-bg rounded-lg hover:bg-primary-400 transition-colors">Sign In</button>
            </div>
          </div>
        </div>
      )}
