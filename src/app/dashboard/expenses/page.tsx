...
export default function ExpensesPage() {
  ...
  const saveExpense = () => {
    ...
    if (editingExpense) {
      setExpenses(expenses.map(e => e.id === editingExpense.id ? { ...formData, id: editingExpense.id, createdAt: editingExpense.createdAt } : e))
    } else {
  ...
  const openEditForm = (expense: any) => {
    ...
    setFormData({ date: expense.date, category: expense.category, description: expense.description, vendor: expense.vendor || '', amount: expense.amount, paymentMethod: expense.paymentMethod, notes: expense.notes || '', isDeductible: expense.isDeductible })
    setShowForm(true)
  }
    ...
    <div className="animate-fade-in pb-8">
      ...
      <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { resetForm(); setShowForm(true) }} className="px-5 py-2.5 text-sm font-semibold bg-primary-500 text-dark-bg rounded-lg hover:bg-primary-400 transition-colors">+ Add Expense</button>
        </div>
      </div>
        ...
        {filteredExpenses.length === 0 ? (
          ...
          filteredExpenses.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(expense => (
            <div 
              ...
              className="group flex items-center gap-4 bg-gradient-to-br from-dark-card to-[#1F1F1F] rounded-xl p-4 border border-dark-border hover:border-primary-500/30 transition-all cursor-pointer hover:shadow-lg hover:shadow-primary-500/5"
            >
              ...
              <button onClick={(e) => { e.stopPropagation(); deleteExpense(expense.id) }} className="text-gray-600 hover:text-[#E74C3C] transition-colors opacity-0 group-hover:opacity-100">🗑️</button>
            </div>
          ))
        )}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4 backdrop-blur-sm overflow-y-auto">
          <div className="bg-gradient-to-br from-dark-card to-[#1F1F1F] rounded-2xl p-6 max-w-lg w-full border border-dark-border my-8">
            <h2 className="font-playfair text-xl text-primary-400 mb-6">{editingExpense ? '✏️ Edit Expense' : '➕ Add Expense'}</h2>
            ...
            <div className="flex gap-4 justify-end">
              ...
              <button onClick={saveExpense} className="px-6 py-3 text-sm font-semibold bg-primary-500 text-dark-bg rounded-lg hover:bg-primary-400 transition-colors">{editingExpense ? 'Save Changes' : 'Save Expense'}</button>
            </div>
          </div>
        </div>
      )}
