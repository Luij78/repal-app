      ...
      <div className="space-y-4">
        {upcomingAppointments.length === 0 ? (
          ...
          upcomingAppointments.map(apt => {
            ...
            return (
              <div key={apt.id} className={`flex gap-4 bg-gradient-to-br from-dark-card to-[#1F1F1F] rounded-xl p-5 border border-dark-border relative ${isPast ? 'opacity-60' : ''}`}>
                {/* Date Badge */}
                ...
                {/* Delete Button */}
                <button onClick={() => deleteAppointment(apt.id)} className="absolute top-3 right-3 text-2xl text-gray-600 hover:text-white transition-colors leading-none">×</button>
              </div>
            )
          })
        )}
