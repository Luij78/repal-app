...
export default function MileagePage() {
  ...
  const saveTrip = () => {
    ...
    if (editingTrip) {
      setTrips(trips.map(t => t.id === editingTrip.id ? { ...formData, id: editingTrip.id, createdAt: editingTrip.createdAt } : t))
    } else {
  ...
  const openEditForm = (trip: any) => {
    setEditingTrip(trip)
    setFormData({ date: trip.date, purpose: trip.purpose, startLocation: trip.startLocation || '', endLocation: trip.endLocation || '', miles: trip.miles, tripType: trip.tripType })
    setShowForm(true)
  }
    ...
    <div className="animate-fade-in pb-8">
      ...
      <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { resetForm(); setShowForm(true) }} className="px-5 py-2.5 text-sm font-semibold bg-primary-500 text-dark-bg rounded-lg hover:bg-primary-400 transition-colors">+ Log Trip</button>
        </div>
      </div>
        ...
        {filteredTrips.length === 0 ? (
          ...
          filteredTrips.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(trip => (
            ...
            >
              ...
              <button onClick={(e) => { e.stopPropagation(); deleteTrip(trip.id) }} className="text-gray-600 hover:text-[#E74C3C] transition-colors opacity-0 group-hover:opacity-100">🗑️</button>
            </div>
          ))
        )}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-gradient-to-br from-dark-card to-[#1F1F1F] rounded-2xl p-6 max-w-lg w-full border border-dark-border">
            <h2 className="font-playfair text-xl text-primary-400 mb-6">{editingTrip ? '✏️ Edit Trip' : '➕ Log Trip'}</h2>
            ...
            <div className="flex gap-4 justify-end">
              ...
              <button onClick={saveTrip} className="px-6 py-3 text-sm font-semibold bg-primary-500 text-dark-bg rounded-lg hover:bg-primary-400 transition-colors">{editingTrip ? 'Save Changes' : 'Save Trip'}</button>
            </div>
          </div>
        </div>
      )}
